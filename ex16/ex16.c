...
struct Person Stack_create(char *name, int age, int height, int weight)
{
    struct Person who;
...
void Stack_print(struct Person who)
{
    printf("Name: %s\n", who.name);
    printf("\tAge: %d\n", who.age);
    printf("\tHeight %d\n", who.height);
...
int main(int argc, char *argv[])
{
    ...
    // now to make stack person
    printf("------\n");
    printf("Making stack person\n");
    struct Person lucy = Stack_create(
        "Lucy Hattersley", 29, 170, 69);

    printf("Lucy is at memory location %p:\n", &lucy);
    Stack_print(lucy);
